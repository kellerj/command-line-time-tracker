...
const ttAdd = new Command()
  ...
  .parse();
const options = ttAdd.opts();
LOG(`Parsed ttAdd Object: ${JSON.stringify(ttAdd, null, 2)}`);

// Build the new entry object with command line arguments
const newEntry = {
  entryDescription: ttAdd.args.join(' ').trim(),
...
// newEntry.entryDate = getEntryDate(ttAdd);
// newEntry.minutes = getEntryMinutes(ttAdd);
